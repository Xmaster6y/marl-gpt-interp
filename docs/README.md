...
- **Launch:** all `nwq@v100` jobs were cancelled at zero runtime on 2026-07-23. The local `jz` remote and canonical WORK checkout now use `jhr`; the old `nwq` WORK checkout was removed after its durable outputs were verified in the new location. Runtime setup job `83591` is priority-pending on `prepost`, and the claim-bearing suite remains gated on setup plus the A100 CUDA/model smoke.
