...
```bash
uv run -m ipykernel install --user --name=marl-gpt-interp --display-name "🔬 MARL GPT Interp"
```
...
```bash
uv run jupyter console --kernel=marl-gpt-interp # --debug
```
...
```
uv run jupyter kernelspec uninstall marl-gpt-interp
```
